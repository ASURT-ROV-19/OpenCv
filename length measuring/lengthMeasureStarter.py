...
#cap=cv2.VideoCapture("udpsrc port=5000 ! application/x-rtp,media=video,payload=26,clock-rate=90000,encoding-name=JPEG,framerate=30/1 ! rtpjpegdepay ! jpegdec ! videoconvert ! videoscale ! video/x-raw,height=720,width=1280 ! appsink",cv2.CAP_GSTREAMER);
#cap=cv2.VideoCapture("udpsrc port=5000 ! application/x-rtp,media=video,payload=26,clock-rate=90000,encoding-name=JPEG,framerate=30/1 ! rtpjpegdepay ! jpegdec ! videoconvert ! videoscale ! video/x-raw,height=720,width=1280 ! appsink",cv2.CAP_GSTREAMER);
...
while True:
    ...
    if key==ord('s'):
        ...
        if(ret==1):
	    imageName="/home/abdelrahman/.PyCharmEdu2018.3/config/scratches/frame"+str(i)+".jpg"
